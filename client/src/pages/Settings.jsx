...
const inputClass = 'mt-2 block w-full rounded-lg border border-gray-700 bg-gray-900 px-4 py-3 text-sm text-gray-100 outline-none transition focus:border-green-400 disabled:cursor-not-allowed disabled:opacity-60';
...
const SettingsCard = ({ title, description, children, actions }) => (
  <section className="rounded-xl border border-gray-700 bg-gray-800 p-5 sm:p-6">
    <div className="flex flex-col gap-4 border-b border-gray-700 pb-4 sm:flex-row sm:items-start sm:justify-between">
      <div>
...
const CheckboxGroup = ({ options, value, onChange }) => (
  <div className="grid gap-3 sm:grid-cols-2">
    {options.map((option) => {
      ...
      return (
        <label key={option.value} className="flex items-center gap-3 rounded-xl border border-gray-700 bg-gray-900 p-3 text-sm text-gray-300">
          <input
            type="checkbox"
            checked={checked}
            onChange={(event) => {
            ...
            className="h-4 w-4 rounded border-gray-700 bg-gray-950 text-green-500"
          />
          {option.label}
        </label>
      );
...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchSettingsData = async () => {
      ...
      try {
        ...
        setProfile(profileFromUser(profileResponse.data, savedSettings));
  ...
  const saveLocalSection = (section, message = 'Settings saved') => {
  ...
  const handleExportData = async () => {
    setExporting(true);
    try {
      const [profileResult, accountsResult, tradesResult, rulesResult, reviewsResult] = await Promise.allSettled([
        api.get('/users/profile'),
        api.get('/accounts'),
        api.get('/trades'),
        api.get('/rules'),
        api.get('/weekly-reviews'),
      ]);
      ...
      const exportData = {
        ...
        trades: getData(tradesResult, []),
        rules: getData(rulesResult, []),
        weeklyReviews: getData(reviewsResult, []),
  ...
  const comingSoon = (label) => {
    toast(`${label} will be available when the backend endpoint is added.`);
  };
        ...
        <div className="mb-6 rounded-xl border border-gray-700 bg-gray-900 p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full border border-gray-700 bg-gray-950">
                {avatarPreview || profile.profilePhotoUrl ? (
                  ...
                  <UserRound size={34} className="text-gray-500" />
                )}
              </div>
              <div>
                <h3 className="font-bold text-gray-100">Profile photo</h3>
                <p className="mt-1 text-sm leading-6 text-gray-500">Upload a JPG, PNG, WEBP, or GIF up to the configured image limit.</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <input
            ...
            className="mt-4 block w-full text-sm text-gray-300 file:mr-4 file:rounded-lg file:border-0 file:bg-gray-700 file:px-4 file:py-2 file:font-semibold file:text-gray-100 hover:file:bg-gray-600"
          />
        </div>
  ...
  const renderTrading = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('trading', 'Trading preferences saved'); }} className="space-y-5">
      <SettingsCard
  ...
  const renderRisk = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('risk', 'Risk management settings saved'); }} className="space-y-5">
      <SettingsCard
  ...
  const renderJournal = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('journal', 'Journal preferences saved'); }} className="space-y-5">
      <SettingsCard
      ...
      >
        <div className="space-y-6">
          ...
          <div>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-[0.18em] text-gray-500">Required trade fields</h3>
            <CheckboxGroup
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-900 p-4">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="font-bold text-gray-100">AI Settings</h3>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
            ...
            <button type="button" onClick={() => comingSoon('Clear AI review history')} className="mt-4 rounded-lg border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-900 p-4">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="font-bold text-gray-100">Mentor Settings</h3>
            </div>
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="grid gap-3">
                ...
                <button type="button" onClick={() => comingSoon('Leave mentor group')} className="rounded-lg border border-gray-700 px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700">
                  Leave mentor group
                </button>
              </div>
            </div>
  ...
  const renderNotifications = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('notifications', 'Notification settings saved'); }}>
      <SettingsCard
  ...
  const renderAppearance = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('appearance', 'Appearance settings saved'); }}>
      <SettingsCard
      ...
      >
        <div className="grid gap-5 md:grid-cols-2">
          <SelectInput label="Theme" value={settings.appearance.theme} onChange={(event) => updateSettingsSection('appearance', 'theme', event.target.value)}>
  ...
  const renderSecurity = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('security', 'Security settings saved'); }} className="space-y-5">
      <SettingsCard
      ...
      >
        <div className="grid gap-5 lg:grid-cols-[1fr_0.9fr]">
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-gray-700 bg-gray-900 p-4">
              <h3 className="font-semibold text-gray-100">Active session</h3>
              <p className="mt-2 text-sm text-gray-500">Current browser session, secured with an HTTP-only auth cookie.</p>
            </div>
            <button type="button" onClick={() => comingSoon('Active session history')} className="w-full rounded-lg border border-gray-700 px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700">
              View active sessions
            </button>
            <button type="button" onClick={() => comingSoon('Account activity history')} className="w-full rounded-lg border border-gray-700 px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700">
              Account activity history
            </button>
          </div>
          <div className="rounded-xl border border-gray-700 bg-gray-900 p-4">
            <KeyRound size={24} className="text-green-400" />
            <h3 className="mt-3 font-bold text-gray-100">Change password</h3>
            <p className="mt-2 text-sm leading-6 text-gray-500">Start a password reset flow for {profile.email || 'your account email'}.</p>
            <button type="button" onClick={handlePasswordReset} disabled={savingSection === 'password'} className="mt-4 w-full rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-gray-900 hover:bg-green-400 disabled:opacity-70">
              {savingSection === 'password' ? 'Sending...' : 'Send reset instructions'}
            </button>
            <button type="button" onClick={() => comingSoon('Log out of all devices')} className="mt-3 w-full rounded-lg border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">
              Log out of all devices
            </button>
            <button type="button" onClick={() => comingSoon('Two-factor authentication')} className="mt-3 w-full rounded-lg border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">
  ...
  const renderBilling = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('billing', 'Billing preferences saved'); }}>
      <SettingsCard
      ...
      >
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-xl border border-green-500/20 bg-green-500/10 p-5">
            ...
            <h3 className="mt-3 text-3xl font-black text-white">{user?.subscriptionPlan || 'FREE'}</h3>
            <p className="mt-2 text-sm text-gray-400">Status: {user?.subscriptionStatus || 'ACTIVE'}</p>
            <p className="mt-4 text-sm text-gray-500">Renewal date: Not scheduled on Free plan</p>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {['Upgrade plan', 'Billing history', 'Payment method', 'Cancel subscription'].map((label) => (
                <button key={label} type="button" onClick={() => comingSoon(label)} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">
                  {label}
                </button>
              ))}
            </div>
            <div className="rounded-xl border border-gray-700 bg-gray-900 p-4 text-sm text-gray-400">
  ...
  const renderDataPrivacy = () => (
    <form onSubmit={(event) => { event.preventDefault(); saveLocalSection('dataPrivacy', 'Data and privacy settings saved'); }} className="space-y-5">
      <SettingsCard
      ...
      >
        <div className="space-y-5">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <button type="button" onClick={() => comingSoon('Screenshot download archive')} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">
              Download screenshots
            </button>
            <button type="button" onClick={() => comingSoon('Account-only export')} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">
              Export account data
            </button>
            <button type="button" onClick={() => comingSoon('Delete individual trading account data')} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">
              Delete account data
            </button>
          </div>
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
              <input
                ...
                placeholder="Type DELETE to prepare destructive actions"
                className="rounded-lg border border-red-500/30 bg-gray-950 px-4 py-3 text-sm text-gray-100 outline-none focus:border-red-300"
              />
              <button type="button" onClick={() => comingSoon('Delete all journal data')} disabled={deleteConfirmation !== 'DELETE'} className="rounded-lg border border-red-400/40 px-4 py-3 text-sm font-semibold text-red-200 hover:bg-red-500/10 disabled:cursor-not-allowed disabled:opacity-50">
                Delete journal data
              </button>
              <button type="button" onClick={() => comingSoon('Delete account')} disabled={deleteConfirmation !== 'DELETE'} className="rounded-lg bg-red-500 px-4 py-3 text-sm font-bold text-white hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-50">
                Delete account
              </button>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => comingSoon('Privacy Policy page')} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">Read Privacy Policy</button>
            <button type="button" onClick={() => comingSoon('Terms of Service page')} className="rounded-lg border border-gray-700 px-4 py-3 text-sm text-gray-300 hover:bg-gray-700">Read Terms of Service</button>
          </div>
        </div>
      </SettingsCard>
    </form>
  ...
  return (
    <div className="space-y-6 text-gray-100">
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        <aside className="hidden rounded-xl border border-gray-700 bg-gray-800 p-3 lg:block">
        ...
        <div className="min-w-0">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-200">{activeLabel}</h2>
          </div>
